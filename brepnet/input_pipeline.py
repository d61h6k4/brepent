...
import tfds_fusion_360_gallery_dataset.cad.fusion360gallery.fusion360gallery
...
def get_datasets(batch_size: int) -> dict[str, tf.data.Dataset]:
  ...
  if batch_size <= 1:
    raise ValueError("Batch size must be > 1 to account for padding graph.")
...
def convert_to_graph_tuples(graph: dict[str, tf.Tensor]) -> jraph.GraphsTuple:
  ...
  labels = tf.concat(
      (graph["face_labels"], 8 * tf.ones(graph["n_node"] - faces_num, tf.uint32)), axis=0)
