...
class MLP(nn.Module):
  ...
  def __call__(self, inputs):
    ...
    for size in self.feature_sizes:
      ...
      x = self.activation(x)
      x = nn.Dropout(
          rate=self.dropout_rate, deterministic=self.deterministic)(
              x)
    return x
  ...
  def __call__(self, graphs: jraph.GraphsTuple) -> jraph.GraphsTuple:
    ...
    for _ in range(self.message_passing_steps):
      ...
      graph_conv = jraph.GraphConvolution(
          update_node_fn=update_node_fn, add_self_edges=False)
    ...
    decoder = jraph.GraphMapFeatures(
        embed_node_fn=nn.Dense(self.output_globals_size))
    processed_graphs = decoder(processed_graphs)
