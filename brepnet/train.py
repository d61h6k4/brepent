...
def remove_labels(graph: jraph.GraphsTuple) -> jraph.GraphsTuple:
    """Remove labels from the training graph."""
    return graph._replace(nodes=graph.nodes["features"])
...
def train_step(
    ...
    def loss_fn(params, graphs):
        ...
        # Extract labels.
        labels = graphs.nodes["labels"]

        # Replace the global feature for graph classification.
        graphs = remove_labels(graphs)
    ...
    for step in range(initial_step, config.num_train_steps + 1):
        ...
        if step % config.eval_every_steps == 0 or is_last_step:
            ...
            for split in splits:
                writer.write_scalars(
                    step,
                    add_prefix_to_keys(eval_metrics[split].compute(), split))
